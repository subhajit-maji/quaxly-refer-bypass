...
# ---------- Rate limiter (5 requests per 5 minutes → 1 per 60 seconds) ----------
solver_lock = threading.Lock()
last_solver_time = 0.0
MIN_SOLVE_INTERVAL = 60   # seconds
...
# ---------- Turnstile token (rate‑limited) ----------
def get_turnstile_token():
    global last_solver_time
    with solver_lock:
        now = time.time()
        wait = MIN_SOLVE_INTERVAL - (now - last_solver_time)
        if wait > 0:
            logging.info(f"Rate limit – waiting {wait:.0f}s before next solver request")
            time.sleep(wait)
        last_solver_time = time.time()

    try:
        resp = requests.post(
            SOLVER_URL,
            json={"mode":"turnstile-max","siteKey":SITE_KEY,"url":"https://panel.quaxly.com/auth/register"},
            timeout=60
        )
...
# ---------- Test runner (concurrency forced to 1) ----------
...
def run_load_test(total, concurrency, ref):
    ...
    # Force concurrency to 1 – rate limiter makes parallel workers pointless
...
# Keep templates (they are fine)
# Restart service
subprocess.run(['systemctl', 'restart', 'quaxly-tester'], check=True)

print("\n✅ Rate‑limited panel deployed (1 solve / 60 seconds).")
print("   Open: http://57.158.25.135")
print("   Secret code: SM")
print("   Concurrency forced to 1. Each signup takes ~60-90 seconds.")
PYEOF